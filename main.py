...
and options while using a sleek UI integrated into Maya.
"""
...
class TxConverterUI(QtWidgets.QDialog):
    def __init__(self, parent=get_maya_main_window()):
        ...
        # Use a frameless window but remove translucent attribute (solid background fixes overlay issues in Maya)
        self.setWindowFlags(QtCore.Qt.FramelessWindowHint)
        # Remove WA_TranslucentBackground so the window shows a solid background instead of overlaying Maya's viewport
        # self.setAttribute(QtCore.Qt.WA_TranslucentBackground)  <-- Removed

        # Define colors (from your modern UI sample)
        self.COLORS = {
            "background": "#2D2D2D",
            "surface": "#2B2B2B",
            "content_bg": "#363636",
        ...
        # Variables for frameless window interactions
        ...
        # Set a solid background color for the container to cover the entire dialog
        ...
        self.title_bar.setStyleSheet("background-color: {}; border-radius: 8px 8px 0 0;".format(self.COLORS["surface"]))
        ...
        # Install an event filter on the title bar so it can be used for moving the window.
        ...
        # Output Field (logs and grouped textures)
        ...
        # Place the content widget inside a scroll area for scrollability
    ...
    # ---------------------------
    # Event Filter for Title Bar (to move the window)
    # ---------------------------
    ...
    def determine_color_space(self, filename, extension, tif_srgb):
        """
        Determine color space based on the file name and extension.
        Priority:
          1) If name already contains _raw, _srgb_texture, or _lin_srgb, use that.
          2) Else if the name matches a data pattern, mark as raw.
          3) Else if .exr then lin_srgb.
          4) Else if .tif/tiff then use TIF checkbox.
          5) Else default to srgb_texture.
        """
    ...
    def convert_texture_to_tx(self, texture, color_space, additional_options,
                              ...
                              use_compression, use_renderman):
        """
        Convert a texture to .tx (for Arnold) or .tex (for RenderMan).
        """
