...
def detect_aces_version(config_path):
    ...
    version_10_markers = [
        "An ACES config generated from python",
        "ACES - ACES2065-1",
        "Output - Rec.709",
    ]
...
class TextureWorker(QtCore.QObject):
    ...
    def __init__(
        ...
        self.arnold_filter = arnold_filter or "lanczos3"
    ...
    def run(self):
        ...
        for i in range(0, total, batch_size):
            ...
            with ThreadPoolExecutor(max_workers=batch_size) as executor:
                futures = {
                    executor.submit(self.convert_texture, tex, csp, opts): (
                        tex,
                        csp,
                    )
    ...
    def convert_texture(self, texture, color_space, additional_options):
        self.logSignal.emit(
            "Starting conversion for {}...".format(os.path.basename(texture))
        )
        arnold_path = os.environ.get("MAKETX_PATH", "maketx")
        color_config = os.environ.get(
            "OCIO", ""
        )  # We rely on "OCIO" by default here
        renderman_path = os.environ.get("RMANTREE", "")
        txmake_path = (
            os.path.join(renderman_path, "bin", "txmake")
            if renderman_path
            else None
        )
        ...
        if ext in ["tex", "tx", "b2r"]:
            self.logSignal.emit(
                "Skipping already-processed file: {}".format(texture)
            )
            return
        ...
        if self.rename_to_acescg:
            ...
            base_name = re.sub(
                r"(_raw|_srgb_texture|_lin_srgb|_acescg)$",
                "",
                base_name,
                flags=re.IGNORECASE,
            )
        ...
        is_displacement = re.search(
            r"_disp|_displacement|_zdisp", base_name, re.IGNORECASE
        )
        is_bump = re.search(r"_bump|_height", base_name, re.IGNORECASE)
        is_normal = re.search(
            r"_normal|_nrm|_norm(?=[^a-z])", base_name, re.IGNORECASE
        )
        ...
        if self.use_renderman and txmake_path:
            self.logSignal.emit(
                "Converting {} to RenderMan .tex...".format(
            ...
            try:
                ...
                self.logSignal.emit(
                    f"Converted to {out_ext}: {texture} -> {out_file}"
                )
            except subprocess.CalledProcessError as e:
                self.logSignal.emit(
                    "Failed to convert {} to .tex: {}".format(texture, e)
                )
            return
        ...
        self.logSignal.emit(
            f"Converting {os.path.basename(texture)} to Arnold .tx..."
        )
        try:
            result = subprocess.run(
            ...
            self.logSignal.emit(
                "maketx output: " + result.stdout.decode("utf-8").strip()
            )
            if result.stderr:
                self.logSignal.emit(
                    "maketx errors: " + result.stderr.decode("utf-8").strip()
                )
...
class TxConverterUI(QtWidgets.QDialog):
    def __init__(self, parent=get_maya_main_window()):
        ...
        self.arnold_maketx_filter = "lanczos3"
        ...
        self.container.setStyleSheet(
            f"background-color: {self.COLORS['background']}; border-radius: 8px;"
        )
        ...
        title_label.setStyleSheet(
            f"color: {self.COLORS['text']}; font-size: 14px; font-weight: bold;"
        )
        ...
        self.content_widget.setStyleSheet(
            f"background-color: {self.COLORS['content_bg']};"
        )
        ...
        folder_label = QtWidgets.QLabel(
            "Select folder to load and group textures:"
        )
        folder_label.setStyleSheet(
            f"color: {self.COLORS['text']}; font-size: 12px;"
        )
        ...
        self.include_subfolders_checkbox = QtWidgets.QCheckBox(
            "Include Subfolders"
        )
        ...
        self.compression_checkbox.setStyleSheet(
            f"color: {self.COLORS['text']};"
        )
        self.compression_checkbox.setChecked(True)
        content_layout.addWidget(self.compression_checkbox)

        self.add_suffix_checkbox = QtWidgets.QCheckBox(
            "Add missing color space suffix"
        )
        self.add_suffix_checkbox.setStyleSheet(
            f"color: {self.COLORS['text']};"
        )
        self.add_suffix_checkbox.setChecked(False)
        content_layout.addWidget(self.add_suffix_checkbox)

        self.renderman_checkbox = QtWidgets.QCheckBox(
            "Convert to RenderMan .tex"
        )
        ...
        self.rename_to_acescg_checkbox = QtWidgets.QCheckBox(
            "Rename to ACEScg Color Space"
        )
        self.rename_to_acescg_checkbox.setStyleSheet(
            f"color: {self.COLORS['text']};"
        )
        self.rename_to_acescg_checkbox.setChecked(False)
        content_layout.addWidget(self.rename_to_acescg_checkbox)

        self.renderman_bumprough_checkbox = QtWidgets.QCheckBox(
            "Use Renderman Bump Rough"
        )
        self.renderman_bumprough_checkbox.setStyleSheet(
            f"color: {self.COLORS['text']};"
        )
        self.renderman_bumprough_checkbox.setChecked(False)
        content_layout.addWidget(self.renderman_bumprough_checkbox)

        self.hdri_checkbox = QtWidgets.QCheckBox(
            "HDRI (use 32-bit float for color textures)"
        )
        ...
        tif_label.setStyleSheet(
            f"color: {self.COLORS['text']}; font-size: 12px;"
        )
        content_layout.addWidget(tif_label)

        self.tif_srgb_checkbox = QtWidgets.QCheckBox(
            "Treat TIF/TIFF color as sRGB (uncheck for linear)"
        )
    ...
    def setup_settings_menu(self):
        ...
        self.settings_menu.addSection("Arnold maketx filter")

        self.arnold_filter_action_group = QtWidgets.QActionGroup(self)
        self.arnold_filter_action_group.setExclusive(True)
        self.arnold_filter_action_group.triggered.connect(
            self.on_arnold_filter_selected
        )
        ...
        for filter_name in filter_options:
            action = QtWidgets.QAction(filter_name, self)
            action.setCheckable(True)
            action.setData(filter_name)
            if filter_name == self.arnold_maketx_filter:
                action.setChecked(True)
            self.arnold_filter_action_group.addAction(action)
            self.settings_menu.addAction(action)

        self.settings_button.setMenu(self.settings_menu)

    def on_arnold_filter_selected(self, action):
        selected_filter = action.data()
        if not selected_filter:
            return
        self.arnold_maketx_filter = selected_filter
        self.log(
            f"Settings updated: Arnold maketx filter = {self.arnold_maketx_filter}"
        )
    ...
    def dropEvent(self, event):
        ...
        if dropped_paths:
            ...
            for f in dropped_paths:
                self.log("  " + f)
            self.log(
                "When you click 'Process Textures', only dropped files will be processed."
            )
        event.acceptProposedAction()
    ...
    def load_textures(self):
        ...
        if not folder_path:
            QtWidgets.QMessageBox.warning(
                self, "Warning", "No folder selected."
            )
            return
        ...
        if not textures:
            QtWidgets.QMessageBox.warning(
                self,
                "Warning",
                "No valid texture files found in the selected folder.",
            )
            return
        ...
        for tex in textures:
            ...
            color_space, additional_options, _ = self.determine_color_space(
                tex, ext, tif_srgb
            )
        ...
        if recurse:
            for root, _, files in os.walk(folder_path):
                for file in files:
                    f_lower = file.lower()
                    # also skip .b2r
                    if f_lower.endswith(valid_exts) and not f_lower.endswith((
                        ".tex",
                        ".tx",
                        ".b2r",
                    )):
    ...
    def rename_dropped_files(self, file_list):
        ...
        for fp in file_list:
            ...
            if any(
                suf in base.lower()
                for suf in ["_raw", "_srgb_texture", "_lin_srgb", "_acescg"]
            ):
                skipped.append(fp)
                updated.append(fp)
            else:
    ...
    def process_textures(self):
        if self.dropped_files:
            ...
            if self.add_suffix_checkbox.isChecked():
                self.log(
                    "Adding missing color space suffixes to dropped file(s)..."
                )
                self.dropped_files = self.rename_dropped_files(
                    self.dropped_files
                )
            ...
            if not folder_path:
                QtWidgets.QMessageBox.warning(
                    self, "Warning", "No folder path found."
                )
                return
            ...
            if add_suffix_selected:
                ...
                self.rename_files(
                    folder_path,
                    add_suffix=add_suffix_selected,
                    recurse=recurse,
                )
        ...
        if skipped_textures:
            self.log(
                f"Skipped textures (unrecognized color space): {len(skipped_textures)}"
            )
        ...
        self.worker = TextureWorker(
            ...
            use_compression,
            use_renderman,
            arnold_filter=self.arnold_maketx_filter,
            hdri_mode=hdri_mode,
        ...
        if event.buttons() & QtCore.Qt.LeftButton:
            if getattr(self, "_is_resizing", False):
                ...
                new_geo.setWidth(
                    max(
                        self.minimumWidth(),
                        self._resize_start_geo.width() + delta.x(),
                    )
                )
                new_geo.setHeight(
                    max(
                        self.minimumHeight(),
                        self._resize_start_geo.height() + delta.y(),
                    )
                )
